...
.offer-box{background:#FFFBEB;border-left:3px solid #D4AF37;padding:8px 12px;border-radius:0 6px 6px 0;font-size:12px;color:#412402;margin-top:8px;}
.offer-box.expiring{background:#FEF2F2;border-left-color:#C0392B;color:#991B1B;}
</style>
...
for k, v in [("payload",None),("shortlist",[]),("dq_notes",[]),("last_updated",None)]:
                ...
                for i, h in enumerate(hotels[:visible]):
                    with h_cols[i % 3]:
                        ...
                        st.markdown(
                            ...
                            f'{offer_html}</div>', unsafe_allow_html=True)
...
with tab_shortlist:
    if not st.session_state.shortlist:
        ...
        for r in sl:
            ...
            rows.append({
                "#": r["rank"], "Hotel": r["hotel"], "Region": r["region"],
                "Board": r["board"],
        ...
        def highlight_urgent(row):
            return ["background-color:#FEF2F2"]*len(row) if "⚡" in str(row.get("Book By","")) else [""]*len(row)
        st.dataframe(df_sl.style.apply(highlight_urgent, axis=1), use_container_width=True, hide_index=True)
        st.download_button("⬇️ Download shortlist CSV", data=df_sl.to_csv(index=False),
